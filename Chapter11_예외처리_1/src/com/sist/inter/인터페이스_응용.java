...
class MoleGame implements Game
{
	...
	public void start() {
		...
		//게임 => 5번 : 1~3 => 선택 입력 =>
		System.out.print("👾 두더지 게임 시작!!");
		System.out.println("1~3사이 두더지가 나오면 위치를 맞추세요");
		for(int i = 1 ; i <= 5 ; i++)
		{
			int mole = rand.nextInt(3)+1; // 0~2
			// 1~3
			System.out.println("\n[라운드 "+i+"]");
			...
			//걸린 시간
			long time = endTime-startTime; //1000 -> 1초
			
			if(time > 2000)
			{
				System.out.println("⏰ 너무 느림! 실패");
				continue; // i++ 증감식으로 돌아감
			}
...
class SpeedGame implements Game
{
	...
	public void start() {
		...
		System.out.println("👍 지금! Enter를 누르세요!!");
		System.out.println("🤦‍♂️ 빨리 누르면 실패");
		try {
			//CheckException => 반드시 예외처리
			System.out.println("\n👻준비...");
			Thread.sleep(rand.nextInt(3000)+2000); // 2~5초 사이
		} catch (Exception ex) {}
 ...
 * 
 * 		c.a = 10; 변수는 A클래스 변수
 * 		c.display => B클래스의 display() 메소드는 생성자 따라
 */
...
//		game.start();
		...
		//상속과 포함에서만 가능 (객체 비교)
//		if(mole instanceof Game) //mole이 Game안에 포함되어있는지  객체의 크기 비교
//		{
...
//		if(sb instanceof String) //상송이나 포함관계가 아니기 때문에 오류
//		{
//			
//		}
