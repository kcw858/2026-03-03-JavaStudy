...
/*
 *  람다: 익명의 함수 => 코드 블록으로 값처럼 전달할 수 있게 만들어 준것
 *  --
 *   1) 기본 문법
 *    	()-> {실행문}
 *    		  ------ 실행문이 1개면 {} 생략 가능
 ...
 *    = 코드가 짧고 사용이 편리
 *    = 함수형 프로그래밍 스타일 가능
 *    	---------> 독립적으로 사용이 가능 -> 메소드는 클래스내에서 사용해야하는데 
 *    								  람다는 클래스외에서 선언이 가능
 *    	
 *    	기존의 방식 => 익명의 클래스 (extends,implements 없이 메소드 재정의가 가능)
 *      //Runnable은 인터페이스
...
//		new Thread(r).start();
		...
		// 조건 => 인터페이스에 구현이 안된 메소드(추상메소드)가 1개만 존재해야한다
